...
def cart_subscription(request):
    if (request.method == 'POST'):
        if request.user.is_authenticated:
            return redirect(reverse("cart_subscription:payment"))
        else:
            return redirect(reverse("accounts:login"))

    cart_subscriptions = Cart_for_Subscription.objects.filter(user=request.user)
